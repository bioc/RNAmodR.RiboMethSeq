...
nomenclature of [[@Birkedal.2015,@Galvanin2019]](#References) with the names `scoreRMS`
(default), `scoreA`, `scoreB` and `scoreMean`.
...
annotation <- GFF3File(RNAmodR.files[["example_RMS.gff3"]])
sequences <- FaFile(RNAmodR.files[["example_RMS.fasta"]])
files <- list("Sample1" = c(treated = RNAmodR.files[["example_RMS1.bam"]]),
              "Sample2" = c(treated = RNAmodR.files[["example_RMS2.bam"]]))
```
...
```{r}
table <- read.csv2(RNAmodR.files[["snoRNAdb.csv"]],
...
```{r plot4, fig.cap="TPR vs. FPR plot.", fig.asp=1}
plotROC(msrms,coord)
```
